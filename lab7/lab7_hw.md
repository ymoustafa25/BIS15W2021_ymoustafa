...
```r
amniota %>% 
  summarize(numbnas = sum(is.na(amniota)))
```
...
```r
amphibio %>% 
  summarize(numbnas =sum(is.na(amphibio)))
```
...
```
It makes sense for mammalia to have 100% NA's, because mammals don't lay eggs (except for a few exceptions). For birds and reptiles, it would sense to have 50% NA's given that only one sex lays eggs (half the population of birds/reptiles). However the reptilia NA's are 91% of the data, which might suggest a sampling bias for male reptiles. 
...
## 4 Ter        1104     16.3
```
Yes, because an animal of a species most likely does not fall into all of those categories because it does not exhibit all of these characteristics. 

**10. Now that we know how NA's are represented in the `amniota` data, how would you load the data such that the values which represent NA's are automatically converted?**

```r
amniota <- readr::read_csv("data/amniota.csv", na = c("-999", "--30258.711"))
```
